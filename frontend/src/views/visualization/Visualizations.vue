      ...
      <el-header height="74px">
        <div class="header">
          <el-form ref="queryForm" :inline="true" :model="queryModel">
            ...
            <!-- <el-form-item :label="$t('Visualization.filter.ApprovedDate')"
                          prop="ApproveDate">
              <el-date-picker v-model="queryModel.ApproveDate"
                              type="daterange"
                              :range-separator="$t('label.dateRange.separator')"
                              :start-placeholder="$t('label.dateRange.start')"
                              :end-placeholder="$t('label.dateRange.end')"></el-date-picker>
            </el-form-item> -->
            ...
            <el-button type="primary" @click="resultVisible = true">
              {{ $t("template.result") }}
              <el-icon class="el-icon--right">
                <svg-icon icon-class="operation" />
              </el-icon>
...
import { GetLocationInfos } from "@/api/result";
...
export default defineComponent({
  ...
  setup() {
    ...
    return { queryModel, currentDiagram };
  },
  data() {
    return {
      popUpShow: false,
  ...
  provide() {
    return {
      visualizationQueryFilter: () => this.buildQueryString(),
    };
  },
  mounted() {
    this.updateTreeView();
  ...
  methods: {
    ...
    getQueryStringArray() {
      ...
      if (this.queryModel.Organism) {
        if (Number.isInteger(this.queryModel.Organism)) {
          filterStr.push(`Organism1Id eq ${this.queryModel.Organism}`);
        } else if ((this.queryModel.Organism as string).length > 0) {
          filterStr.push(`Organism1 eq '${this.queryModel.Organism}'`);
        }
      }
    ...
    buildQueryString() {
      const filterStr = this.getQueryStringArray();
      if (filterStr.length > 1) {
        return filterStr.map((f) => `(${f})`).join(" and ");
      } else {
    ...
    onSymbolEnter(symbol: {
      ...
      if (symbol) {
        ...
        this.popupTop = symbol.group.y + mainRef.offsetTop + symbol.parent.y;
        this.popupRight =
          mainRef.offsetWidth - symbol.group.x - symbol.parent.x + 15;
        this.currentSymbol = symbol as any;
        // let popup = this.$refs.popupRef.$el;
        // popup.offsetLeft = 100;


        this.enter();
        this.locationId = symbol.config?.locationId;
        this.mapId = symbol.config.id;
      }
    },
    onPopupShow() {},
    ...
    onDiagramPosChange() {
      if (this.popUpShow && this.currentSymbol) {
        let mainRef = (this.$refs.mainWin as any).$el;
      ...
      if (ids[0] == "1") {
        ...
        this.$query("LocationV", {
          ...
          .then((res) => {
            ...
            // this.location.name = res.value[0]?.Name;

            // for (let index = 0; index < res.value.length; index++) {
            //   if (parseInt(ids[1]) == res.value[index].LocationId) {
            //     this.location.name = res.value[index].name;
            //   }
            //   if (q != "") {
            //     q += " or ";
            //   }
            //   q += `LocationId eq ${res.value[index].LocationId}`;
            // }
            filter.push(
            ...
            if (filter && filter.length > 0) {
              ...
              //   "RptSampleMart",
              //   {
              //     $select: "LocationId,MostServerLimit,LocationFullName",
              //   }
              // ).then((data) => {
              //   this.getLocationdata(data.value);
              // });
        ...
        this.$query("RptSampleMart", {
          ...
          for (let index = 0; index < data.value.length; index++) {
            if (currentTestTypeName != data.value[index].TestTypeName) {
              ...
              singleTest.count = 0;
              // if (
              //   data.value[index].ActionLimitValue != "" &&
              //   data.value[index].ActionLimitValue != null
              // ) {
              //   singleTest.actionValue = data.value[index].ActionLimitValue;
              // }
              // if (
              //   data.value[index].AlertLimitValue != "" &&
              //   data.value[index].AlertLimitValue != null
              // ) {
              //   singleTest.alertValue = data.value[index].AlertLimitValue;
              // }
